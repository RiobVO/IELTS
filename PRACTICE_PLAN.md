...
> **Апдейт 2026-07-21 (`ac16a90`): practice-поверхность получила verbatim-рендер.**
> Атомизированный раннер в practice теперь рендерит оригинальную вопрос-панель
> (`questions_html`: групп-заголовки, инструкции, таблицы) с per-question кластерами
> аффордансов (`PracticeAffordances.tsx` — Check/reveal, format-hint, strategy,
> confidence, вынесены из QuestionBlock в общий компонент). Гейт: слоты
> `.q-slot[data-q]` обязаны покрывать ВСЕ вопросы пассажа (`questionsHtmlCoversAll`),
> иначе прежний атомизированный список (DnD-тесты Inspera фоллбэчат корректно —
> drag-вопросы слотов не имеют). Mock и стратегия A не изменены. Хвост трека:
> drag-слоты/групп-рубрики для verbatim DnD (вариант C аудит-карты) — в BACKLOG.
