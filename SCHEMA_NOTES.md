...
удержания отдельного лока (та же логика, что try-lock в `checkIpThrottle`, но здесь
...
## 0057 — annotation/payment grant lockdown (закрытие drift-серии)

Codex-ревью волны 1.5 указало, что «drift-когорта» контракта (annotation/payment,
гранты не пинились с обоснованием «барьер — RLS») была тавтологией: RLS не
покрывает TRUNCATE/REFERENCES/TRIGGER, так что дрейф на них не инертен. 0057
повторяет 0056 для последних двух таблиц со старым паттерном «GRANT SELECT без
REVOKE ALL» (0006/0013): anon — ноль, authenticated — только policy-scoped SELECT
(клиентское чтение истории платежей на `/app/profile` живо). После 0057 дрейфа не
осталось нигде, и контракт `rls-contract.ts` стал строгим без исключений: режимы
`any`/`knownProdGrantDrift` удалены, anon и PUBLIC проверяются безусловно (ноль
табличных И колоночных грантов на каждой таблице), `selectOnly` требует НАЛИЧИЯ
SELECT (пустые гранты = сломанные клиентские чтения = красный).

## 0058 — profile.referral_cap_bonus (реферальная награда = подъём капа, G1-1)

Growth-волна 1: награда за приглашение переехала с «только XP» на реальный подъём
Basic-капа — `+1 mock-старт/неделю` ОБЕИМ сторонам пары за каждого приглашённого,
дошедшего до rated-первой попытки. Слагаемое хранится денормализованно на `profile`,
а не считается `COUNT`'ом по `referral`: авторитетная проверка капа живёт внутри
транзакции `startAttempt`, где строка `profile` УЖЕ залочена `SELECT ... FOR UPDATE`,
и колонка приезжает тем же чтением — ноль лишних round-trip на горячем пути и ноль
шансов разъехаться с конкурентным начислением (`maybeRewardReferral` ждёт тот же лок).

Потолок (`REFERRAL_MOCK_BONUS_MAX = 3`, `src/lib/tiers.ts`) применяется дважды: в SQL
при инкременте (`LEAST(...)`, чтобы конкурентные награды не перепрыгнули его) и при
чтении (`mockWeeklyLimit`). CHECK-констрейнта сознательно нет — снижение потолка иначе
требовало бы миграции данных вместо правки константы. Колонка наследует RLS-постуру
`profile` (owner-only SELECT, записи owner-path) — новых грантов/политик не нужно,
как у 0049. Миграция бэкфиллит уже награждённые пары, иначе прошлые рефералы молча
потеряли бы награду.

## 0059 — content_item.published_at (свежесть каталога, G1-2)

Бейдж «New» и витрина «новое на этой неделе» считали свежесть по `created_at` —
времени ИМПОРТА. Реальный поток другой: файл заливается ботом, лежит черновиком до
ревью, публикуется днями позже, и в каталог такой тест приходит уже «не новым».
0059 фиксирует момент публикации отдельной nullable-колонкой (у черновика даты
публикации нет, `NOT NULL DEFAULT now()` соврал бы про каждый draft).

`publishTest` проставляет её через `coalesce(published_at, now())` — повторная
публикация (депубликация с возвратом, переимпорт того же теста) дату НЕ сдвигает,
иначе старый тест снова всплывал бы «новинкой». `getPublishedTests` отдаёт
`coalesce(published_at, created_at)` как страховку для строк, которых не коснулся
бэкфилл. Постура наследуется от `content_item` (публичные колонки, column-grants
0035 не затронуты) — новых грантов/политик не нужно.

## 0060 — attempt.share_token (публичная band-карточка, G1-5)

Чтобы расшаренная ссылка разворачивалась превью в Telegram, страница и og:image
обязаны открываться БЕЗ сессии: краулер мессенджера ходит анонимно, под owner-гейтом
превью было бы пустым. Доступ даёт неугадываемый `share_token`, а не `attempt.id` —
публичная ссылка не светит внутренний id, которым адресуются owner-scoped маршруты
(`/result?a=…`), и шеринг отзывается одним `UPDATE ... SET share_token = NULL`.

Выдача — server action `createShareLink` с гейтом ПРЯМО В WHERE (владелец ∧
`status='submitted'`): подделанный id вернёт пустой RETURNING, а не токен. Ленивая и
идемпотентная (`coalesce(share_token, gen_random_uuid())`) — повторный Share отдаёт
ту же ссылку, а не плодит мёртвые URL. Partial UNIQUE-индекс
`attempt_share_token_key` обслуживает единственный публичный запрос и не тащит в
индекс NULL-строки (подавляющее большинство).

Постура `attempt` НЕ меняется (RLS on, клиенту — policy-scoped SELECT своих строк,
0056): публичное чтение по токену идёт owner-path через Drizzle, anon доступа к
таблице не получает. На карточку едут только band/процент, название теста и
слабейший тип — ни ответов, ни ключа, ни персональных данных.

## 0061 — telegram_link (студенческий бот, G2-1)

Связка аккаунта с чатом СТУДЕНЧЕСКОГО Telegram-бота. Отдельная таблица, а не колонка
в `profile`: строка живёт своим циклом (код выдан → обмен → `/stop` удаляет) и держит
короткоживущий секрет, которому не место в профиле, читаемом на каждой странице.

Граница безопасности здесь ИНАЯ, чем у бота импорта (`app/api/telegram/webhook`). Тот
стоит на whitelist'е Telegram user_id, потому что публикует контент; этот открыт всем,
поэтому `chat_id`, пришедший от Telegram, не доказывает ничего. Доказательство создаёт
залогиненный владелец: server action `connectTelegram` выдаёт одноразовый код (128 бит,
TTL 15 минут), в БД лежит только его `sha256` — из строки нельзя восстановить
приглашение даже с доступом на чтение. Обмен атомарен: `UPDATE ... WHERE code_hash = $1
AND code_expires_at > now()` гасит код в том же запросе, поэтому второй обмен тем же
кодом не находит строку. `chat_id` UNIQUE — один чат принадлежит одному аккаунту;
повторная привязка снимает прежнюю.

Постура — owner-стейт как `mistake_review`/`saved_word`: RLS on, `REVOKE ALL` от
клиентских ролей, `GRANT SELECT` + `telegram_link_select_own` (страница профиля читает
состояние связки), записи — только owner-path (server action + вебхук). `verify.ts`
держит её в §4i-когорте (RLS on + anon SELECT denied).

`pending_content_item_id`/`pending_question_number` — вопрос со свободным вводом, на
который бот ждёт ответ текстом; `last_nudge_on` (UTC-день) даёт идемпотентность
ежедневной рассылки: повторный прогон крона в тот же день не пишет второй раз.

## 0062 — passage_content_item_order_key (UNIQUE, техдолг W2-13)

БД-инвариант там, где write-path гарантию лишь стремился дать: обычный путь
`persist.ts` пишет уникальный `"order"` внутри `content_item`, но malformed вход
(например, full-Reading HTML с повторяющимся `data-part`) мог протащить дубликат —
два пассажа на одном слоте дают неопределённый порядок рендера каталога/раннера.
