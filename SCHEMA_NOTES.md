...
БД-инвариант поверх гарантии парсера: `persist.ts` пишет пассажи с уникальным
`"order"` внутри `content_item`, но сама БД слота не держала — два пассажа на одном
слоте дают неопределённый порядок рендера каталога/раннера. Прод проверен на дубли
(0 групп) перед применением. Constraint-only миграция: количество таблиц не меняется
(`APP_TABLE_COUNT` = 37), RLS-постура `passage` не тронута.
