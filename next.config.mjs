...
const nextConfig = {
  ...
  // Брендовый шрифт og-карточки (G1-5) читается с диска в Node-рантайме, но
  // трассировщик его не видел: путь собирается через process.cwd(), поэтому файл в
  // лямбду не попадал и роут падал ENOENT'ом — локально баг невидим (файл лежит в
  // дереве исходников), виден ТОЛЬКО в проде. Включаем ассет в трассировку явно.
  outputFileTracingIncludes: {
    "/s/[token]/opengraph-image": ["./app/_og/**"],
  },
