...
> Listening (Writing/Speaking — AI-фаза позже). Контент загружается админом как
...
/app                    (auth) Dashboard — прогресс, стрики, быстрый старт
  ...
  /writing              Темы (загружены админом) — AI-оценка: «скоро»
  /speaking             Темы — AI-оценка: «скоро»
...
| Бейджи / стрики | ✅ | ✅ | ✅ |
| AI-оценка Writing/Speaking (Phase 3) | — | — | ✅ |
...
### 4.10 Writing / Speaking (заглушки)
- Админ грузит темы. UI и data-model готовы; AI-оценка помечена «скоро».
...
**topic** (Writing/Speaking, Phase 3 — заглушка)
...
- **Phase 3 — AI:** автооценка Writing и Speaking по загруженным темам.
...
2. **Listening-формат:** нет ни одного файла. Аудио + map/form-completion
   добиваем на первом реальном файле (ожидается тот же шаблон).
3. **Лимиты Basic** (N тестов/день) — подобрать при запуске.
4. **Apple/Facebook OAuth:** нужны dev-аккаунты/ключи (Apple Developer — платный).
