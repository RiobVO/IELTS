...
| 2 | Email-блок: **код готов целиком**, полка владельца выполнена 2026-07-07 (Brevo-аккаунт, домен bando.study аутентифицирован, sender `IELTS@bando.study`, Supabase SMTP + Confirm ON, Vercel env + redeploy). **Блокер: активация Brevo-аккаунта** (API даёт 403 `account not yet activated`, запрос в support отправлен). Доводка при активации: (1) удалить ВСЕ `weekly_digest`-строки notification, созданные до активации (2 тестовые от 2026-07-07 + возможные от холостых понедельничных cron-прогонов — письма по ним не уходили из-за 403, а 6-дневный гард/недельный ключ скроют этих юзеров из ближайшей рассылки), (2) живой прогон digest + подтвердить доставку (API-ключ Brevo локально не хранится — попросить владельца передать в shell-env), (3) smoke verify-канала реальной регистрацией. Confirm оставлен ON осознанно — сайт ещё не запущен | §11 | important | ждём support Brevo; кода/деплоя не нужно |
