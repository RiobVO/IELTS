...
const PUBLISH_FAIL_MSG: Record<string, string> = {
  ...
  listening_audio_too_short:
    "Нельзя опубликовать: аудио короче теста — похоже, залита только одна часть. " +
    "Пришли ОДИН файл со всеми частями (P1–P4 склеенными).",
  listening_audio_unverified:
    "Нельзя опубликовать: не удалось прочитать длительность аудио — перезалей файл (mp3/m4a).",
