...
import { buildChartGeometry, CHART_DESKTOP, CHART_MOBILE, fmtDate, type ChartGeometryInput } from "@/lib/progress/chart-geometry";
...
export async function OverviewPanel() {
  ...
  const [
    ...
    supabase
      ...
      .gt("band_score", 0)
      // Practice-прогон — тренировка с мягкими условиями, его band несравним с
      // экзаменационным и завышал бы прогноз; замером считается только mock
      // (рейтинг уже живёт по этому правилу). Через этот запрос кормятся
      // Trajectory, Forecast и R/L в Readiness — все три сознательно
      // переводятся на mock-only.
      .eq("mode", "mock")
...
function TrajectoryHero({
  ...
  // Геометрия (домен + шкалы/пути/координаты для конкретного viewBox) — чистый
  // src/lib/progress/chart-geometry.ts, вынесена оттуда без изменений в формулах.
  // Date.now() — единственный внешний вход, параметризован как nowMs.
  const geomInput: ChartGeometryInput = { trajectory, forecast, targetBand, examDate, nowMs: Date.now() };
  ...
  return (
    <div className="ov-hero" style={S.heroCard}>
      ...
      <div className="ov-chart-wide">
        <TrajectoryChart {...buildChartGeometry(geomInput, CHART_DESKTOP)} />
      </div>
      <div className="ov-chart-narrow">
        <TrajectoryChart {...buildChartGeometry(geomInput, CHART_MOBILE)} />
      </div>
    </div>
  );
}
