...
export default async function ReadingCatalog({
  ...
  // question types available across reading content (for the filter row)
  ...
  const availableTypes = [
    ...new Set((allReading ?? []).flatMap((r) => r.question_types as string[])),
  ].sort();
  ...
  return (
    <main style={S.page}>
      <div style={S.wrap}>
        ...
        <h1 style={S.h1}>Reading</h1>
        <p style={S.sub}>Выбери тест. Фильтруй по части и типам вопросов.</p>

        {/* category filter */}
        <div style={S.filterRow}>
          <Chip href="/app/reading" active={!sp.category} label="Все части" />
          {READING_CATEGORIES.map((c) => (
            <Chip
              key={c}
              href={`/app/reading?category=${c}${sp.q_type ? `&q_type=${sp.q_type}` : ""}`}
        ...
        {availableTypes.length > 0 && (
          <div style={S.filterRow}>
            <Chip
              href={`/app/reading${sp.category ? `?category=${sp.category}` : ""}`}
              active={!sp.q_type}
              label="Все типы"
              subtle
            />
            {availableTypes.map((t) => (
              <Chip
                key={t}
                href={`/app/reading?${sp.category ? `category=${sp.category}&` : ""}q_type=${t}`}
                active={sp.q_type === t}
                label={qtypeLabel(t)}
                subtle
              />
