...
import { PracticeAffordances } from "@/components/exam/PracticeAffordances";
...
export default function ExamRunner({
  ...
  // Вопрос по номеру — для verbatim-пути (QuestionHtml знает лишь номер слота, а
  // аффордансам нужен полный вопрос: qtype/prompt/options). Атомизированный список
  // маппит по q напрямую и этой карты не касается.
  const byNumber = useMemo(() => {
    const m = new Map<number, Question>();
    for (const q of questions) m.set(q.number, q);
    return m;
  }, [questions]);

  // Practice-аффордансы ОДНОГО вопроса для verbatim-панели (тот же PracticeAffordances,
  // что в QuestionBlock). Вызывается QuestionHtml вне tree-мемо → вердикты/ответы
  // свежие. Пропы-колбэки стабильны (useCallback), примитивы per-номер → memo внутри
  // PracticeAffordances пропускает неизменившиеся вопросы. Гейтится isPractice на
  // передаче в QuestionHtml (в mock проп undefined → verbatim без аффордансов).
  const renderAffordances = useCallback(
    (n: number) => {
      ...
      return (
        <PracticeAffordances
          ...
          verdict={checked[k]}
          reveal={revealed[k]}
          checkBusy={!!checkBusy[k]}
          wrongTry={wrongTries[k] ?? 0}
          confidence={confidence[k]}
          canLocate={locatableSet.has(n)}
          onCheck={runCheck}
          onReveal={runReveal}
          onLocate={isListening ? undefined : locatePara}
          onConfidence={setConf}
    ...
    [byNumber, answers, checked, revealed, checkBusy, wrongTries, confidence, locatableSet, runCheck, runReveal, isListening, locatePara, setConf, runLocate],
  );
  ...
  return (
    <div id={`q-${q.number}`} style={{ marginBottom: 12 }}>
      <div style={S.card}>
        ...
        {/* Практис-аффордансы (P1/P2b/P6/P7/P10/P14) — общий компонент с verbatim-путём.
            В mock не рендерится (practice=false → mock-раннер не затронут). showNumber
            здесь не нужен: номер уже на карточке (S.qNum выше). */}
        {practice && (
          <PracticeAffordances
            q={q}
            value={value}
