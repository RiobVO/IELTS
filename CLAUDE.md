...
**Growth-волна 2 (удержание) — 3/4 на проде (2026-08-05, `5843900..1d2d73d`).** Закрыты:
...
общий opt-out с дайджестом). Student-бот (`/api/telegram/student`, миграция 0061
`telegram_link`, крон `telegram-nudge` 14:00 UTC) написан и задеплоен, но ВЫКЛЮЧЕН до
появления `TELEGRAM_STUDENT_BOT_TOKEN`: без него вебхук no-op, крон skip, кнопки в
профиле нет. Вопрос дня берётся только из СВОИХ неотработанных ошибок — вердикт
раскрывает ключ, и для непройденного теста это была бы утечка.

**Открыто:** токен студенческого бота от @BotFather + `setWebhook` (внешний вход),
контент-процесс W2-3 (BACKLOG — ритм пополнения),
