...
**Открыто:** первая настоящая вечерняя рассылка бота (крон 14:00 UTC — сегодня прошёл
до появления env), контент-процесс W2-3 (BACKLOG — ритм пополнения),
...
(breaking-апгрейды drizzle-kit/next). Порядок работы: план с acceptance → «делай» → реализация.
