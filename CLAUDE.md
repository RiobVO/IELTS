...
аккаунты/vocab/W/S/saved_word целы (BRIEF §12.3 п.3). Storage-сироты (mp3, source-html)
не чищены — BACKLOG OPS-1; дамп со старым контентом хранится до ~2026-08-09.
Следующее по §12.3 — п.3 контент-процесс W2-3 (BACKLOG; процессный:
ритм пополнения + витрина «новое»). Порядок: план с acceptance → «делай» от
