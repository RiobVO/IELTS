...
**Открыто:** вечерний прогон крона на новом коде (14:00 UTC); verbatim-practice для
Volume 6 — чекбокс-группа с ПО-ВОПРОСНЫМИ ключами (`25:'B', 26:'E'`), существующий
group-slot-механизм пишет общий набор во всех членов и сломал бы грейдинг, нужен свой
аффорданс; стоимость вечернего крона (до 300 получателей последовательно, у каждого
`getOpenMistakes` тянет до 300 снимков попыток — мина на после-анонса);
контент-процесс W2-3 (BACKLOG), merchant-ключи (внешний гейт платежей → волна 0b),
Listening-аудио от клиента (4 драфта; у опубликованного C21 Test 1 озвучена только
Part 1), два `?src=`-линка в посты каналов, Dependabot moderate (drizzle-kit/next).
