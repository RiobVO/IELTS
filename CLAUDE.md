...
- **[TESTING_PLAN.md](./TESTING_PLAN.md)** — трек зрелости тестирования по волнам (аудит
  ...
  ручные `npm run test:hosted:*` против тест-проекта, 2026-07-20) и 3a (stateful
  Playwright-сьют на тест-стенде: раннер+сид+auth/reading/mock-iframe/cap, e2e-БД
  напрямую `db.<ref>` мимо обоих пулеров Supavisor, ×5 зелёные, 2026-07-20).
  Следующая 0b ждёт merchant-ключей; 3b/3c/4 по триггерам. Статус — §14.
...
npm run test:e2e:stateful     # stateful Playwright-сьют против тест-стенда (волна 3a: сид+гейт внутри,
                              # 10 тестов; ручной, один прогон за раз; runbook — TESTING_PLAN §9)
```
