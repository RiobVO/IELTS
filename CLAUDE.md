...
- **Storage-гигиена (BACKLOG OPS-1, закрыто):** `scripts/storage-orphans.ts` (dry-run/`--delete`,
  gitignored, минутный grace-период против гонки с runner-импортом) вычистил 758.5 MB
...
Storage is 1 GB on Supabase Free; `scripts/storage-orphans.ts` (gitignored) inventories/deletes objects
