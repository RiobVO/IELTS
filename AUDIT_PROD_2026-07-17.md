...
## Статус условий (приписка 2026-08-09)

Все 3 major-условия сняты последующими волнами — вердикт фактически GO:

- **#1 restore-drill validator** — сниппет в `docs/RESTORE.md` читает число таблиц
  динамически из `verify.ts::APP_TABLE_COUNT` (`EXPECTED_TABLE_COUNT = Number(tableCountMatch[1])`);
  прозаические «34» и шапка `SCHEMA_NOTES.md` исправлены (закрывает и minor #9).
- **#2 e2e в прод** — `e2e/global-setup.ts` первым делом бросает без `ALLOW_STATEFUL_E2E=1`
  и проверенного не-прод таргета (`e2e/stateful-gate.ts`, волна 3a тест-трека); штатный
  `npm run test:e2e` до service-role записи не доходит.
- **#3 vm OOM** — исполнение импортированного JS вынесено в worker_threads-изолят с
  `resourceLimits.maxOldGenerationSizeMb` (`extract-js.ts`); heap-бомба убивает только
  воркер, репро аудита закреплено тестом (#20 в `extract-js.test.ts`).

Minor/nit: #9, #10 (aria-current), #16 (мёртвая env) — закрыты; #8 разрешён в пользу
кода (кап 12 MB — доки выровнены 2026-08-09); #4 (signup TOCTOU) и #11 (Esc у Session
goal) — закрыты волной доводки 2026-08-09; #6 (digest-dedup) — осознанный skip: крон
1×/день, дубль возможен только при авторизованном ручном retry; #12 → триггерный
Dependabot-хвост (BACKLOG); #5, #7, #13, #15, #17 — приняты как есть. P1–P2 — проверка
владельца в дашбордах; P3 снят фактом: дайджест ошибок приходит в Telegram ежедневно.
