...
- Один mp3 на весь тест (аудио прикладывается к первому passage — не к каждому вопросу
  отдельно).
- Лимит размера — **15 MB** (жёсткий кап и при встроенном `<audio src>` в HTML, и при
  отдельной заливке mp3-файлом через бота — `src/lib/import/audio-cap.ts`).
- Рекомендация по битрейту — **≤64–96 kbps mono**: полный Listening-тест (~30+ минут) на
  64 kbps mono укладывается в ≈14.4 MB, оставляя запас до лимита. Несжатый/стерео исходник
  (128 kbps+) в лимит не влезает — пережимайте перед заливкой.
