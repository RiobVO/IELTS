...
НАШ SHELL (Next.js parent, наш origin)
  ...
  • рендерит <iframe sandbox="allow-scripts allow-same-origin"
...
| Sandbox | `allow-scripts allow-same-origin` | localStorage + аудио требуют same-origin; контент доверенный (из админки) |
